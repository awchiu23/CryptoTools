...
def krRelOrder(side,kr,pair,trade_qty,maxChases=0):
  ...
  if side == 'BUY':
    limitPrice = round(krGetBid(kr, pair)+0.1,1)
    z='Bidding'
  else:
    limitPrice = round(krGetAsk(kr, pair)-0.1,1)
    z='Offering'
  ...
  while True:
    ...
    if (side=='BUY' and newPrice > limitPrice) or (side=='ASK' and newPrice < limitPrice):
      limitPrice=newPrice
      nChases+=1
