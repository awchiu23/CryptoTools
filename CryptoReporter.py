...
class core:
  ...
  def ftxInit(self):
    def cleanBorrows(ccy, df):
      ...
      return df2
    ######
    def getBorrowsLoans(wallet, payments, ccy):
      start_time = getYest()
  ...
  def dbInit(self):
    ...
    notional = futures['FutDeltaUSD'].abs().sum()
    #####
    oneDayIncome = getOneDayIncome('BTC', self.spotBTC) + getOneDayIncome('ETH', self.spotETH)
    oneDayAnnRet = oneDayIncome * 365 / notional
    #####
    nav = spotDeltaBTC * self.spotBTC + spotDeltaETH * self.spotETH
...
if CRYPTOTOOLS_MODE>0 and not IS_IP_WHITELIST:
...
externalCoinsNAV = EXTERNAL_BTC_DELTA * spotBTC + EXTERNAL_ETH_DELTA * spotETH
externalEURNAV = EXTERNAL_EUR_DELTA*(spotEUR-EXTERNAL_EUR_REF)
